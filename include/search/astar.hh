#ifndef SEARCH_ASTAR_HH
#define SEARCH_ASTAR_HH
...
private:
	...
	std::function<bool(const Cell &, const Cell &)> cell_cmp = [this](const kd::Cell &a,
		...
		return m_grid->at(a) > m_grid->at(b);
	};

	std::priority_queue<kd::Cell, std::vector<kd::Cell>, decltype(cell_cmp)> m_frontier;

public:
	ASTAR(const std::shared_ptr<kd::Agent> &agent, const std::shared_ptr<kd::Grid> &grid)
		...
		m_grid->at(m_cur) = m_grid->dist(m_cur, m_goal) + BlockState::VISIT;
	}
