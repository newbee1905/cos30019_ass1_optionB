...
#define ENUM_STR(ENUM_NAME, ...)                                                                   \
	...
	constexpr std::array<std::string_view, kd::va_count(__VA_ARGS__)> __tag_##ENUM_NAME =            \
			kd::split<kd::va_count(__VA_ARGS__)>(#__VA_ARGS__);                                          \
	constexpr std::string_view GetString##ENUM_NAME(ENUM_NAME index) {                               \
		return __tag_##ENUM_NAME[index];                                                               \
	}                                                                                                \
                                                                                                   \
	constexpr std::size_t GetLengthEnum##ENUM_NAME() { return kd::va_count(__VA_ARGS__); }           \
                                                                                                   \
...
#define ENUM_MAP(ENUM_NAME, ...)                                                                   \
	enum ENUM_NAME { __VA_ARGS__ };                                                                  \
	constexpr std::array<kd::pair<std::string_view, ENUM_NAME>, kd::va_count(__VA_ARGS__)>           \
			__tag_map_##ENUM_NAME = kd::split_enum<kd::va_count(__VA_ARGS__), ENUM_NAME>(#__VA_ARGS__);  \
	static constexpr auto __map_##ENUM_NAME =                                                        \
			kd::map<std::string_view, ENUM_NAME, kd::va_count(__VA_ARGS__)>{{__tag_map_##ENUM_NAME}};    \
	constexpr std::string_view GetString##ENUM_NAME(ENUM_NAME index) {                               \
